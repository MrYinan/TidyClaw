# HEARTBEAT.md - robot-cleaner heartbeat policy

heartbeat 是 robot-cleaner 的**低频主动触发层**。

它不负责一步步移动机器人，也不负责把任务拆成短执行段续跑。正式循环巡视交给 `patrol-runner` continuous 执行。heartbeat 主动触发时只做秒级检查：空闲时感知环境；发现可清扫地面目标后用 `patrol-runner` skill 的 `--command start` 启动后台 runner 并立即返回；后续 heartbeat 或用户询问状态时再用 `patrol-runner --command report/status` 汇报结果。
...
```bash
python skills/heartbeat-watchdog/scripts/heartbeat_watchdog.py
```

根据返回值处理：

- `heartbeat_runner_alive`：runner 正在运行，回复 `NO_REPLY`
- `heartbeat_idle_no_task`：空闲且未发现任务，回复 `NO_REPLY`
- `heartbeat_idle_no_scan`：空闲但未执行感知，回复 `NO_REPLY`
- `heartbeat_target_found_started`：主动发现可清扫地面目标，并已通过 `patrol-runner --command start` 启动后台巡视
- `heartbeat_target_found_start_failed`：主动发现目标，但后台巡视启动失败
...
heartbeat-watchdog
  -> 低频检查 runner / 空闲环境
  -> 空闲且发现可清扫目标时启动 patrol-runner

patrol-runner
  -> continuous 循环执行
  -> get-vision -> analyze-scene-opencv -> decide -> move/clean -> state-manager
  -> status/report 返回任务摘要
```

因此：

- heartbeat 负责“空闲时要不要主动启动任务”
...
3. 如果发现可清扫地面目标，启动 `patrol-runner`
...
2. 继续调用 `analyze-scene-opencv`
3. 如果结构化分析发现 `floor_trash_detected = true`，watchdog 调用 `patrol-runner` skill 入口启动后台 runner：

```bash
python skills/patrol-runner/scripts/patrol_runner_skill.py --command start --max-steps 200
```

4. watchdog 返回 `heartbeat_target_found_started`，OpenClaw Agent 只把 `user_message` 发给用户，例如“已启动当前房间自动巡视，后台 runner 正在执行”
5. 如果没有发现可清扫地面目标，保持 `IDLE` 并记录 heartbeat 日志
...
```text
heartbeat -> heartbeat-watchdog -> patrol-runner skill --command start -> 立即返回启动通知
