...
export default defineToolPlugin({
    ...
    tools: (tool) => [
        tool({
            ...
            description: "Refresh robot observation/perception and build the bounded decision context for one model decision turn.",
        ...
        tool({
            ...
            description: "Execute one option_id selected from the current robot_cleaner_decision_context_v1 after local validation.",
        ...
        tool({
            ...
            description: "Return the current robot service task state, patrol state, holding state, and recent progress without executing a physical action.",
            parameters: Type.Object({}, { additionalProperties: false }),
            execute: async (_params, config, context) => {
                context.signal?.throwIfAborted();
                return callRobotToolBackend({
        ...
        tool({
            ...
            description: "Return a user-facing structured report for the current household service robot run without executing a physical action.",
            parameters: Type.Object({}, { additionalProperties: false }),
            execute: async (_params, config, context) => {
                context.signal?.throwIfAborted();
                return callRobotToolBackend({
...
function assertSafeOptionId(optionId) {
